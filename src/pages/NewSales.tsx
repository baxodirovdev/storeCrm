...
export const NewSales = () => {
  ...
  cart.forEach((item: { cart: number; cost: number }) => {
  ...
  const addSaleHandler = () => {
    if (cart.length > 0 && client) {
      let clientInfo = {};
      clients.forEach((item: { key: string }) => {
        if (item.key === client) {
          clientInfo = item;
        }
      });
  ...
  return (
    <Fragment>
      ...
      <ContentHeader className="newSales__header">
        ...
        >
          <Select placeholder="Select a client" onChange={(e) => setClient(e)}>
            {clients.map((client: { key: string; clientName: string }) => {
              return (
      ...
      >
        {cart.map(
          (
            item: { productName: string; cost: number; cart: number },
            index: number
          ) => {
            return (
